...
# Find the V16.10 dashboard even if GitHub added (1) or (2) to the file name.
def find_dashboard_file():
    exact = ROOT / "Adaptive_Inverter_Victory_Showcase_V16_10.html"
    if exact.exists():
        return exact
    matches = sorted(ROOT.glob("Adaptive_Inverter_Victory_Showcase_V16_10*.html"))
...
def full_power_constraint_table(full_table: pd.DataFrame):
    ...
    ai_v = metric_value(full_table, "RMS voltage", "AI")
    fx_v = metric_value(full_table, "RMS voltage", "Fixed")
    ai_i = metric_value(full_table, "RMS current", "AI")
    fx_i = metric_value(full_table, "RMS current", "Fixed")
    ai_p = metric_value(full_table, "Output power", "AI")
    fx_p = metric_value(full_table, "Output power", "Fixed")
    ai_thd = metric_value(full_table, "THD", "AI")
    fx_thd = metric_value(full_table, "THD", "Fixed")

    rows = [
        ["RMS voltage", "215.6–224.4 V (±2%)", fmt(ai_v,2," V"), pass_fail(215.6 <= ai_v <= 224.4), fmt(fx_v,2," V"), pass_fail(215.6 <= fx_v <= 224.4)],
        ["RMS current", "≤ 30 A", fmt(ai_i,2," A"), pass_fail(ai_i <= 30.0), fmt(fx_i,2," A"), pass_fail(fx_i <= 30.0)],
        ["Output power", "≤ 5 kW", fmt(ai_p,1," W"), pass_fail(ai_p <= 5000.0), fmt(fx_p,1," W"), pass_fail(fx_p <= 5000.0)],
        ["THD", "≤ 5%", fmt(ai_thd,3,"%"), pass_fail(ai_thd <= 5.0), fmt(fx_thd,3,"%"), pass_fail(fx_thd <= 5.0)],
...
def full_power_figure(table: pd.DataFrame):
    ...
    for i, (metric, _) in enumerate(items):
        ...
        if metric == "THD":
            fig.add_hline(y=5, line_dash="dash", line_color="#d84949", annotation_text="5% limit", row=r, col=c)
...
def run_complete_analysis(preset, PF, R, Tamb, study_s, fixed_kHz, ea, progress=gr.Progress()):
    ...
    return summary, constraints, metrics, fig_full, fig_sup, full_conv, candidates
...
def build_live_ui():
    with gr.Blocks(title="Adaptive Inverter — Live Interactive", css=LIVE_CSS) as live:
        gr.HTML("<div class='topnav'><a class='backlink' href='/dashboard'>← V16.10 Dashboard</a><b>Live Interactive</b></div>")
        ...
        run.click(run_complete_analysis, inputs=[preset,pf,r,ambient,study,fixed,ea], outputs=[summary,constraints,metrics_table,full_plot,sup_plot,convergence,candidates])
    return live
...
def dashboard():
    if DASHBOARD_FILE is None or not DASHBOARD_FILE.exists():
        return HTMLResponse("<h2>V16.10 HTML not found.</h2><p>Upload Adaptive_Inverter_Victory_Showcase_V16_10.html to the repository root.</p>", status_code=404)
    html = DASHBOARD_FILE.read_text(encoding="utf-8", errors="ignore")
    # Serve the original V16.10 directly — NO iframe. Only add one floating navigation button.
    if "</body>" in html.lower():
        idx = html.lower().rfind("</body>")
        html = html[:idx] + FLOAT_NAV + html[idx:]
    else:
        html += FLOAT_NAV
