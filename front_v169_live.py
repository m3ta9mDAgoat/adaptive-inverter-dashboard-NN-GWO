...
# Load only the V16.12 computational engine definitions.
# IMPORTANT: app.py also constructs the older Gradio UI at module import time.
# Building both UIs can delay Render port binding and consume unnecessary RAM.
# Here we execute app.py only up to its `APP = build_app()` line, preserving
# the ANN/GWO/Supervisor backend while skipping the old presentation layer.
import types
...
def _load_backend_only():
    ...
    exec(compile(backend_source, str(app_path), "exec"), module.__dict__)
    return module

engine = _load_backend_only()
