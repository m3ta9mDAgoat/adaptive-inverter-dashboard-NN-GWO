...
def get_engine(progress=None):
    global _ENGINE
    if _ENGINE is None:
        if progress is not None:
            try:
                progress(0.01, desc="Loading ANN model and backend — first run only")
            except Exception:
                pass
        _ENGINE = load_backend_only()
    return _ENGINE
...
# Keep the UI lightweight at startup. The actual backend/model is loaded lazily
# on the first preset load or live simulation, after Render has already opened its port.
PRESETS = PRESET_PRIORITY.copy()
...
def normalize_frequency_khz(v):
    """Normalize a backend switching-frequency value to kHz.

    The live backend historically returned kHz in its results table, but this
    also safely handles a Hz-valued result (e.g. 6000 -> 6.0 kHz).
    """
    x = _num(v)
    if not np.isfinite(x):
        return np.nan
    return x / 1000.0 if abs(x) > 100.0 else x


def selected_frequency_banner(full_table, sup_table, fixed_kHz):
    # Complete-supervisor output is the final command. Fall back to the
    # frequency-only table if a backend version omits it from the supervisor table.
    raw = metric_value(sup_table, "Switching frequency", "AI")
    source = "Complete supervisor"
    if not np.isfinite(raw):
        raw = metric_value(full_table, "Switching frequency", "AI")
        source = "Frequency-only result"

    ai_f = normalize_frequency_khz(raw)
    fixed_f = normalize_frequency_khz(fixed_kHz)

    if not np.isfinite(ai_f):
        return """
        <div class='freq-banner missing'>
          <div class='freq-kicker'>AI SELECTED SWITCHING FREQUENCY</div>
          <div class='freq-value'>N/A</div>
          <div class='freq-sub'>The backend did not return the selected-frequency metric for this run.</div>
        </div>
        """

    delta = fixed_f - ai_f if np.isfinite(fixed_f) else np.nan
    same = np.isfinite(fixed_f) and abs(ai_f - fixed_f) < 1e-6
    if same:
        action = "No adaptation required — AI selected the fixed reference frequency."
        badge = "NO CHANGE REQUIRED"
    elif np.isfinite(fixed_f):
        direction = "reduced" if ai_f < fixed_f else "increased"
        action = f"Adaptive action: {fixed_f:.2f} → {ai_f:.2f} kHz ({direction} by {abs(delta):.2f} kHz)."
        badge = "ADAPTIVE ACTION"
    else:
        action = "Adaptive frequency selected by ANN + GWO supervisor."
        badge = "AI COMMAND"
    ...
    <div class='freq-banner'>
      <div class='freq-left'>
        <div class='freq-kicker'>AI SELECTED SWITCHING FREQUENCY</div>
        <div class='freq-value'>{ai_f:.2f} <span>kHz</span></div>
        <div class='freq-sub'>Final command source: {source}</div>
      </div>
      <div class='freq-right'>
        <div class='freq-badge'>{badge}</div>
        <div class='freq-ref'>Fixed reference: <b>{fixed_f:.2f} kHz</b></div>
        <div class='freq-action'>{action}</div>
      </div>
    </div>
    """
...
def run_complete_analysis(preset, PF, R, Tamb, study_s, fixed_kHz, ea, progress=gr.Progress()):
    ...
    freq_banner = selected_frequency_banner(full_table, sup_table, fixed_kHz)
    ...
    return freq_banner, summary, constraints, electrical_fig, metrics, fig_full, fig_sup, full_conv, candidates
...
.freq-banner{display:grid;grid-template-columns:.75fr 1.25fr;gap:22px;align-items:center;background:linear-gradient(125deg,#061a33,#0c6a8f);color:#fff;border-radius:22px;padding:23px 28px;margin:4px 0 18px;box-shadow:0 18px 48px rgba(7,38,68,.18);border:1px solid rgba(255,255,255,.12)}.freq-kicker{font-size:11px;font-weight:900;letter-spacing:.12em;color:#bfefff}.freq-value{font-size:52px;line-height:1;font-weight:950;margin:9px 0 8px}.freq-value span{font-size:23px;font-weight:800}.freq-sub{font-size:12px;color:#d4e8f4}.freq-right{border-left:1px solid rgba(255,255,255,.22);padding-left:22px}.freq-badge{display:inline-block;padding:7px 10px;border-radius:999px;background:rgba(73,226,188,.15);border:1px solid rgba(104,239,204,.42);color:#a8f4df;font-size:10px;font-weight:900;letter-spacing:.08em}.freq-ref{font-size:17px;margin:12px 0 7px}.freq-action{font-size:15px;line-height:1.5;color:#e6f3fa;font-weight:700}.freq-banner.missing{background:#6a301f}.freq-banner.missing .freq-value{font-size:36px}
#run-btn{background:#0b1f3a!important;color:white!important;font-weight:850!important;min-height:50px!important;border-radius:12px!important}footer{display:none!important}@media(max-width:1050px){.decision-grid{grid-template-columns:1fr 1fr 1fr}}@media(max-width:720px){.decision-grid{grid-template-columns:1fr 1fr}.freq-banner{grid-template-columns:1fr}.freq-right{border-left:0;border-top:1px solid rgba(255,255,255,.22);padding-left:0;padding-top:16px}.freq-value{font-size:43px}}
"""


def build_live_ui():
    with gr.Blocks(title="Adaptive Inverter — Live Interactive", css=LIVE_CSS) as live:
        gr.HTML("<div class='topnav'><button class='backlink' type='button' onclick=\"window.top.location.href='https://neural-network-gwo-adaptive-inverter.netlify.app/'; return false;\">← V16.10 Dashboard</button><b>Live Interactive</b></div>")
        ...
        run.click(run_complete_analysis, inputs=[preset,pf,r,ambient,study,fixed,ea], outputs=[freq_banner,summary,constraints,electrical_plot,metrics_table,full_plot,sup_plot,convergence,candidates])
    return live


live_ui = build_live_ui()
api = FastAPI(title="Adaptive Inverter Live Interactive")

# Keep the dashboard external on Netlify.
@api.get("/dashboard")
def dashboard():
    return RedirectResponse(
        url="https://neural-network-gwo-adaptive-inverter.netlify.app/",
        status_code=302,
    )

# Preserve older links that pointed to /live or /live/.
@api.get("/live")
def live_redirect():
    return RedirectResponse(url="/", status_code=302)

@api.get("/live/")
def live_redirect_slash():
    return RedirectResponse(url="/", status_code=302)

# Mount Gradio directly at the Render root so the service URL always opens
# the interactive page without a sub-path/root_path mismatch.
api = gr.mount_gradio_app(api, live_ui, path="/")
